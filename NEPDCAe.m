function x = NEPDCAe(A,b,lambda,t,x0)
%proposed by Lu, Zhou (Algorithm 3)
epsilon = 1e-6;
eta=1e-3;
theta = zeros(200,1);
beta_v = zeros(200,1); 
...
for iter=1:2000   
    ...
    for i=1:Num
        ...
        z=max(s*gx-gf+alpha*bx-s,min(0,s*gx-gf+alpha*bx+s))/(alpha);
        hzt = sum(max(z-t,0)+max(0,-z-t));
        F_rho = norm(A*z-b)^2/2 + s*(norm(z,1)-hzt)+c*(z-x)'*(z-x)/2;
        if F_rho<funv
            x_new=z;
            funv=F_rho;
        end
